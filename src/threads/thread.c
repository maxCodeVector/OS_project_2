...
// declare some function only used in this file
void init_process(struct process *proc);
struct process_node* create_process_node(struct process *proc);
                    ...
                    thread_func *function, void *aux)
{
  ...
  struct process_node* node = create_process_node(&t->proc);
  t ->node = node;
  list_push_back(&cur->proc.child, &t->node->child_elem);
...
struct process_node* create_process_node(struct process *proc){
  // malloc process memory
  // note we can not get it by threads page, 
  // since when thread destroy father can not kown its return value
